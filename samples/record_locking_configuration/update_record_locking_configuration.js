...
class UpdateRecordLockingConfiguration {
	...
	static async updateRecordLockingConfiguration(id, moduleName) {
		...
		let response = await recordLockingConfigurationOperations.updateRecordLockingConfiguration(id, bodyWrapper);
		if (response != null) {
...
await UpdateRecordLockingConfiguration.initialize();
let moduleAPIName = "Deals";
let id = 34770938001n;
await UpdateRecordLockingConfiguration.updateRecordLockingConfiguration(id, moduleAPIName);