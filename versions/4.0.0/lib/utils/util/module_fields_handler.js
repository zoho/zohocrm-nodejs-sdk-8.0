...
import {JSONConverter} from "../../zohocrmsdk.js";
...
class ModuleFieldsHandler {
  ...
  static async getDirectory() {
    ...
    return join(initializer.getResourcePath(), Constants.FIELD_DETAILS_DIRECTORY);
  }
  ...
  static async deleteFieldsFile() {
    try {
      let recordFieldDetailsPath = await new JSONConverter().getEncodedFileName()
    ...
    try {
      ...
      if (existsSync(recordFieldDetailsPath)) {
        ...
        if (recordFieldDetailsJson.hasOwnProperty(module.toLowerCase())) {
          ...
          writeFileSync(recordFieldDetailsPath, JSON.stringify(recordFieldDetailsJson));
        }
      }
    } catch (error) {
      let exception = new SDKException(null, null, null, error);
  ...
  static async refreshFields(module) {
    try {
      ...
      await Utility.getFieldsInfo(module).catch((err) => { throw err; });
    } catch (error) {
