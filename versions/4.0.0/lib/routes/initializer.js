...
class Initializer {
	...
	static async getInitializer() {
		if (Initializer.initializer != null && Array.from(Initializer.LOCAL.keys()).length > 0) {
			let encodedKey = await Initializer.initializer.getEncodedKey().catch(err => {throw err;});
	...
	static async switchUser(environment, token, sdkConfig, proxy = null) {
		...
		initializer._token = token;
		// compute the key from the new initializer before it replaces the global, not after
		let encodedKey = await initializer.getEncodedKey().catch(err => {throw err;});
		Initializer.LOCAL.set(encodedKey, initializer);
		Initializer.initializer = initializer;
	...
	static async removeUserConfiguration(token) {
		if (token instanceof OAuthToken) {
			if (token.getId() == Initializer.initializer.getToken().getId()) {
				let key = await Initializer.initializer.getEncodedKey();
	...
	async getEncodedKey() {
		// derive the key from this instance, not the currently-active global initializer
		...
		let tokenKey = this.getEnvironment().getUrl();
	...
	async toString() {
		return (Constants.IN_ENVIRONMENT).concat((await Initializer.getInitializer()).getEnvironment().getUrl()).concat(".");
	}
