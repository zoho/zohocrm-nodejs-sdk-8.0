...
class GetDownloadAttachmentDetails {
    ...
    static async getDownloadAttachmentDetails(module, recordId, userId, messageId, destinationFolder) {
        let downlaodAttachmentOperations = new ZOHOCRMSDK.DownloadAttachments.DownloadAttachmentsOperations();
        let paramInstance = new ZOHOCRMSDK.ParameterMap();
        await paramInstance.add(ZOHOCRMSDK.DownloadAttachments.GetDownloadAttachmentsDetailsParam.MESSAGE_ID, messageId);
        await paramInstance.add(ZOHOCRMSDK.DownloadAttachments.GetDownloadAttachmentsDetailsParam.USER_ID, userId);
        ...
        if (response != null) {
            ...
            if (responseObject != null) {
                if (responseObject instanceof ZOHOCRMSDK.DownloadAttachments.FileBodyWrapper) {
                    let streamWrapper = responseObject.getFile();
                    let fileName = path.join(destinationFolder, streamWrapper.getName());
                    let readStream = streamWrapper.getStream();
                    fs.writeFileSync(fileName, readStream);
                }
                else if (responseObject instanceof ZOHOCRMSDK.DownloadAttachments.APIException) {
