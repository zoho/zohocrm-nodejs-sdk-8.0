...
class GetRelatedEmails {
    ...
    static async getRelatedEmails(moduleAPIName, id) {
        let emailRelatedOperations = new ZOHOCRMSDK.EmailRelatedRecords.EmailRelatedRecordsOperations(id, moduleAPIName);
        let paramInstance = new ZOHOCRMSDK.ParameterMap();
        await paramInstance.add(ZOHOCRMSDK.EmailRelatedRecords.GetEmailsRelatedRecordsParam.INDEX, "");
        ...
        if (response != null) {
            ...
            if (responseHandler instanceof ZOHOCRMSDK.EmailRelatedRecords.ResponseWrapper) {
                let responseWrapper = responseHandler;
                let emailTemplates = responseWrapper.getEmails();
                if (emailTemplates != null) {
                    emailTemplates.forEach(emailTemplate => {
                        if (emailTemplate instanceof ZOHOCRMSDK.EmailRelatedRecords.Email) {
                            ...
                            if (status != null) {
                                status.forEach(status1 => {
                                    if (status1 instanceof ZOHOCRMSDK.EmailRelatedRecords.Status) {
            ...
            else if (responseHandler instanceof ZOHOCRMSDK.EmailRelatedRecords.APIException) {
