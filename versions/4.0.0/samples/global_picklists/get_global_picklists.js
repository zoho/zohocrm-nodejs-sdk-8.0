...
class GetGlobalPicklists {
	...
	static async getGlobalPicklists() {
		let sendMailsOperations = new ZOHOCRMSDK.GlobalPicklists.GlobalPicklistsOperations();
		let paramInstance = new ZOHOCRMSDK.ParameterMap();
		// await paramInstance.add(ZOHOCRMSDK.GlobalPicklists.GetGlobalPicklistsParam.INCLUDE, null);
		// await paramInstance.add(ZOHOCRMSDK.GlobalPicklists.GetGlobalPicklistsParam.INCLUDE_INNER_DETAILS, "used_in_modules.plural_label");
		let response = await sendMailsOperations.getGlobalPicklists(paramInstance);
		if (response != null) {
...
await GetGlobalPicklists.initialize();
await GetGlobalPicklists.getGlobalPicklists();