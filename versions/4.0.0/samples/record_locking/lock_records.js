...
class LockRecords {
    ...
    static async lockRecords(recordId, moduleAPIName) {
        ...
        let response = await recordLockingOperations.lockRecords(bodyWrapper);
        if (response != null) {
...
await LockRecords.initialize();
await LockRecords.lockARecord(recordId, moduleAPIName);
