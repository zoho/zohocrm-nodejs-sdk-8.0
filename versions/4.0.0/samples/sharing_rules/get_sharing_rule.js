...
class GetSharingRule{
    ...
    static async getSharingRule(moduleAPIName, id) {
        const sharingRulesOperations = new ZOHOCRMSDK.SharingRules.SharingRulesOperations(moduleAPIName);

        const response = await sharingRulesOperations.getSharingRule(id);
...
await GetSharingRule.initialize();
await GetSharingRule.getSharingRule("Leads", 3231232n);