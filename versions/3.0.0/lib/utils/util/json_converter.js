...
class JSONConverter extends Converter {
  ...
  async isRecordRequest(recordInstance, classDetail, instanceNumber, memberDetail) {
    ...
    for (let keyName of Array.from(keyModified.keys())) {
      ...
      if (keyValue != null) {
        if (keyDetail != null && Object.keys(keyDetail).length > 0) {
          if ((keyDetail.hasOwnProperty(Constants.READ_ONLY) && (keyDetail[Constants.READ_ONLY] == true || keyDetail[Constants.READ_ONLY] == "true")) || !keyDetail.hasOwnProperty(Constants.NAME)) {
            // read only or no keyName
            continue;
          }
    ...
    if (skipMandatory === true || this.checkException(classMemberName, recordInstance, instanceNumber, lookUp, requiredKeys, primaryKeys, new Map())) {
      return requestJSON;
    }
    return requestJSON;
  }
  ...
  async getResponse(responseJson, packageName) {
    ...
    if (classDetail.hasOwnProperty(Constants.INTERFACE) && classDetail[Constants.INTERFACE]){
      ...
      if (instance instanceof Record) {
        ...
        instance = await this.notRecordResponse(instance, responseJson, classDetail).catch((err) => {throw err;}); // based on json details data will be assigned
      }
    }
    return instance;
  }
  ...
  async notRecordResponse(instance, responseJSON, classDetail) {
    for (let memberName in classDetail) {
      let keyDetail = await classDetail[memberName];
        ...
        if (memberDetail.hasOwnProperty(Constants.KEYS)) {
          ...
          for (let keyIndex = 0; keyIndex < keysDetail.length; keyIndex++) {
            ...
            if (response.hasOwnProperty(keyName) && response[keyName] != null) {
              ...
              keys = keys.filter(key => key !== keyName);
            }
          }
        }
        if (keys.length > 0) {
    ...
    if (responses.length > 0) {
      if ( memberDetail == null || (memberDetail != null && !memberDetail.hasOwnProperty(Constants.STRUCTURE_NAME))) {
        if(memberDetail != null && memberDetail.hasOwnProperty(Constants.SUB_TYPE)){
          ...
          for (let response of responses) {
            values.push(
                await this.redirectorForJSONToObject(response).catch((err) => { throw err; })
            );
          }
				}
      } 
  ...
  async getModuleDetailFromUserSpecJSON(module) {
    var recordFieldDetailsPath = join(await this.getEncodedFileName().catch((err) => {throw err;}));
