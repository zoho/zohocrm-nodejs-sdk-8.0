...
import {OAuthToken} from "../../zohocrmsdk.js";
...
class Utility {
  ...
  static async fileExistsFlow(moduleAPIName, recordFieldDetailsPath, lastModifiedTime) {
    ...
    if (initializer.getSDKConfig().getAutoRefreshFields() == true && !this.newFile && !this.getModifiedModules && (!recordFieldDetailsJson.hasOwnProperty(Constants.FIELDS_LAST_MODIFIED_TIME) || this.forceRefresh || new Date().getTime() - recordFieldDetailsJson[Constants.FIELDS_LAST_MODIFIED_TIME] > 3600000)) {
      ...
      initializer.getSDKConfig().getAutoRefreshFields() == false && this.forceRefresh && !this.getModifiedModules) {
      this.getModifiedModules = true;
      await Utility.modifyFields(recordFieldDetailsPath, lastModifiedTime).catch((err) => { throw err; });
      this.getModifiedModules = false;
    }
  ...
  static async verifyModuleAPIName(moduleName) {
    ...
    var recordFieldDetailsPath = await this.getFileName().catch((err) => { throw err; });
    ...
    if (apiPath.toLowerCase().includes(moduleAPIName.toLowerCase())) {
      ...
      for (var i = 0; i < apiPathSplit.length; i++) {
        if (apiPathSplit[i].toLowerCase() == moduleAPIName.toLowerCase()) {
          ...
          Constants.DEFAULT_MODULENAME_VS_APINAME.has(apiPathSplit[i].toLowerCase()) &&
          Constants.DEFAULT_MODULENAME_VS_APINAME.get(apiPathSplit[i].toLowerCase()) != null) {
  ...
  static async getFileName(){
    ...
    var input = new Converter().toUTF8Array(fileName);
  ...
  static async getModules(header) {
    ...
    let response = await new ModuleOperations().getModules(parameterMap, headerMap);
    if (response != null) {
  ...
  async getUserName(token){
    ...
    let userName = null;
		let paramInstance = new ParameterMap();
    await paramInstance.add(GetUsersParam.TYPE, Constants.CURRENTUSER);
