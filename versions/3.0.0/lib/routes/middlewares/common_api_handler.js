...
class CommonAPIHandler {
  ...
  async apiCall(className, encodeType) {
    ...
    try {
      connector.headers.set(Constants.ZOHO_SDK, os.platform() + "/" + os.release() + "/nodejs-8.0/" + process.version + ":" + Constants.SDK_VERSION);
      ...
      if (response.headers.hasOwnProperty(Constants.CONTENT_TYPE_HEADER.toLowerCase())) {
        ...
        if (responseObject != null) {
          ...
          if (responseObject.length == 2 && responseObject[1] != null) {
      ...
      return new APIResponse(headerMap, response.statusCode, returnObject, responseJSON);
    } catch (error) {
      if (!(error instanceof SDKException)) {
        error = new SDKException(null, null, null, error);
      }
