...
class MassDeleteRecordId {
    ...
    static async massDeleteRecordId(moduleAPIName) {
        ...
        let bodyWrapper = new ZOHOCRMSDK.MassDeleteCvid.RecordIdBodyWrapper();
        bodyWrapper.setIds(["4402481177154"]);
        let territory = new ZOHOCRMSDK.MassDeleteCvid.Territory();
        territory.setId(0n);
        ...
        if (response != null) {
            ...
            if (actionHandler instanceof ZOHOCRMSDK.MassDeleteCvid.ActionWrapper) {
                let actionWrapper = actionHandler;
                let actionResponses = actionWrapper.getData();
                if (actionResponses != null) {
                    actionResponses.forEach(actionResponse => {
                        if (actionResponse instanceof ZOHOCRMSDK.MassDeleteCvid.SuccessResponse) {
                        ...
                        else if (actionResponse instanceof ZOHOCRMSDK.MassDeleteCvid.MassDeleteScheduled) {
                            let successResponse = actionResponse;
                            console.log("Status: " + successResponse.getStatus().getValue());
                            console.log("Code: " + successResponse.getCode().getValue());
                            console.log("Details: ");
                        ...
                        if (actionResponse instanceof ZOHOCRMSDK.MassDeleteCvid.APIException) {
            ...
            } else if (actionHandler instanceof ZOHOCRMSDK.MassDeleteCvid.APIException) {
