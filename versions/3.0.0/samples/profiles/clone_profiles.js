...
class CloneProfiles {
    ...
    static async cloneProfiles(profileId) {
        let profilesOperations = new ZOHOCRMSDK.Profiles.ProfilesOperations();
        let bodyWrapper = new ZOHOCRMSDK.Profiles.BodyWrapper();
        let profiles = [];
        ...
        let response = await profilesOperations.cloneProfiles(profileId, bodyWrapper);
        if (response != null) {
...
let profileId = 665390031160n;
await CloneProfiles.initialize();
await CloneProfiles.cloneProfiles(profileId);